...
// draw curve using line segments with GL_LINE_LOOP
    ...
    glEnd();
    /*
     * Desenha MAL as linha derivadas de cada ponto
    for(int i = 0; i < pointCount; i ++){
        getGlobalCatmullRomPoint(i * (1.0f)/pointCount, pos, deriv);
        glBegin(GL_LINES);
...
void renderScene(void){
    ...
    float* xi = (float*) malloc(sizeof(float) * 3);
    float* zi = (float*) malloc(sizeof(float) * 3);
    float* yi = (float*) malloc(sizeof(float) * 3);
    float* yiOld = (float*) malloc(sizeof(float) * 3);
